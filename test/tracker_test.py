...
"""
This is an exact replica of the main module, except that it is linked to the test_db.
It is meant to be run after the Habit class unit test and the Habit class/database integration test.

This is meant to be used to experiment with the GUI and its features, specifically with how it restricts certain
habit class methods and how it can show past data, which is not something one can see with the actual main module
as it is connected to a database with empty tables.

The tables here can be reloaded by running the test_db_integration.py module again.
"""
...
if __name__ == "__main__":
    ...
    def add_habit_popup():
        ...
        def create_new_habit():
            ...
            if period_var.get() == "" or new_habit_name.get() == "" or period_var.get().isspace() or new_habit_name.get().isspace():
                utils.error_popup("Needs name and periodicity!")
            elif new_habit_name.get() in habit_namelist:
                utils.error_popup(f"There already exists a habit named {new_habit_name.get()}!\nPlease choose another name.")
    ...
    #another very extensive function: this one to render the whole analytics module and all its widgets
    def show_analytics():
        """
        ...
        best = tk.Text(analytics_lower_frame, width=50)
        best.grid(column=0, row=0)
        best_habits = []
        for habit in habit_group:
            if habit.streak > 0:
                best_habits.append([habit.name, habit.period, "streak:", habit.streak])
        best.insert("1.0", "The longest currently-running habits are:\n\n")
        best.insert(tk.END, f"{utils.parse_list_for_ui(sorted(best_habits, key=lambda x: x[3], reverse=True), 3)}")

        worst = tk.Text(analytics_lower_frame, width=50)
        worst.grid(column=1, row=0)
        worst_habits = []
        for habit in habit_group:
            if habit.fail_count > 0:
                worst_habits.append([habit.name, habit.period, "breaks:", habit.fail_count])
        worst.insert("1.0", "The habits you struggled the most with are:\n\n")
        worst.insert(tk.END, f"{utils.parse_list_for_ui(sorted(worst_habits, key=lambda x: x[3], reverse=True), 3)}")
        ...
        #to display all habits, categorized based on periodicity
        period_based_list = tk.Text(analytics_upper_frame, width=40)
        period_based_list.grid(column=0, row=0)

        #list for each periodicity
        daily_habits = []
        weekly_habits = []
        monthly_habits = []
        ...
        for habit in habit_group:
            if habit.period == "Daily":
                daily_habits.append([habit.name, "streak:", habit.streak])
            elif habit.period == "Weekly":
                weekly_habits.append([habit.name, "streak:", habit.streak])
            elif habit.period == "Monthly":
                monthly_habits.append([habit.name, "streak:", habit.streak])

        #function parse_list_for_ui from local utils module unpacks the list of lists in a way that is useful for the UI
        period_based_list.insert("1.0", f"Daily Habits:\n{utils.parse_list_for_ui(sorted(daily_habits, key=lambda x: x[2], reverse=True), 2)}\n\n"
                                        f"Weekly Habits:\n{utils.parse_list_for_ui(sorted(weekly_habits, key=lambda x: x[2], reverse=True), 2)}\n\n"
                                        f"Monthly Habits:\n{utils.parse_list_for_ui(sorted(monthly_habits, key=lambda x: x[2], reverse=True), 2)}\n\n")
        ...
        def on_combobox_select(evt):
            ...
            #loops through Log table and returns which action took place when
            #remember: row[0] is habit name, row[1] is date, and row[2] is action
